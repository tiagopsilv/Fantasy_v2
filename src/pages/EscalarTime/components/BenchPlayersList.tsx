...
export interface BenchPlayersListProps {
  benchPlayers: Player[];
...
interface PositionColors {
  [key: string]: string;
}

const POSITION_COLORS: PositionColors = {
  QB: "bg-blue-600",
  RB: "bg-green-600", 
  WR: "bg-purple-600",
  TE: "bg-orange-600",
...
const getPositionColor = (position: string): string => {
  return POSITION_COLORS[position] || "bg-gray-600";
};
...
const BenchPlayerItem = React.memo<BenchPlayerItemProps>(({ 
  player, 
  onRemoveFromBench, 
  onMoveToLineup, 
  isLocked 
}) => {
  const handleMoveToLineup = React.useCallback(() => {
    if (onMoveToLineup && !isLocked) {
      onMoveToLineup(player);
    }
  }, [onMoveToLineup, isLocked, player]);

  const handleRemove = React.useCallback(() => {
    if (!isLocked) {
      onRemoveFromBench(player);
    }
  }, [isLocked, onRemoveFromBench, player]);

  return (
    <div className="flex items-center justify-between p-3 bg-[#1A2238] rounded-lg border border-[#4A4E56] opacity-75">
...
BenchPlayerItem.displayName = "BenchPlayerItem";

export const BenchPlayersList = React.memo<BenchPlayersListProps>(({ 
  benchPlayers, 
  onRemoveFromBench, 
  onMoveToLineup,
  isLocked = false 
}) => {
  const isEmpty = benchPlayers.length === 0;

  return (
    <div className="bg-[#2C2F33] rounded-lg border-2 border-[#FFD700] shadow-lg shadow-[#FFD700]/20 p-4 sm:p-6">
      <div className="mb-4 pb-3 border-b-2 border-[#FFD700]/50">
        <h3 className="text-white text-xl sm:text-2xl font-bold">Banco de Reservas</h3>
      </div>
          ...
          <div className="space-y-3">
            {benchPlayers.map((player) => (
              <BenchPlayerItem
                key={player.id}
                player={player}
                onRemoveFromBench={onRemoveFromBench}
                onMoveToLineup={onMoveToLineup}
                isLocked={isLocked}
              />
            ))}
          </div>
...
BenchPlayersList.displayName = "BenchPlayersList";